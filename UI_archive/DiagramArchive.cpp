...
void DiagramArchive::DoDraw(HDC hdc, HWND hWnd, PAINTSTRUCT ps, RECT& r, HFONT hFont, int& number) {
...
void DiagramArchive::DoParameters(HWND hWnd, wstring str, string global_archive, int& number, HWND hWnd1, HWND hWnd2) {
    openArchive(hWnd);
    ...
    if (GetOpenFileName(&ofn) == TRUE) {
        ...
        amount_size_files = sizeFiles(global_archive, number, size_archive);
...
vector<pair<float, string>> DiagramArchive::sizeFiles(const string path, int& count, float& sizeArch) {
...
void DiagramArchive::ratio(float max, vector<float> vec, vector<int>& hg) {
    vector<int>mnb;
    for (auto i : vec) {
...
void DiagramArchive::openArchive(HWND hWnd) {
...
string DiagramArchive::lastWordFile(string word) {
    vector<string> vec;
